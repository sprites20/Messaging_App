...
# Ensure you have a proper secret key
...
def parse_message_content(content):
    """Parse message content to extract text and image correctly."""
    content_data = json.loads(content)
    
    print("Content Data: ", content_data)
    if isinstance(content_data, list):  # If content is a list (e.g., [{"type": "text", "text": "..."}])
        text_content = next((item["text"] for item in content_data if item.get("type") == "text"), "")
        image_url = next((item["image_url"] for item in content_data if item.get("type") == "image_url"), None)
        if image_url != None:
            if image_url["url"]:
                image_url = image_url["url"]
    elif isinstance(content_data, dict):  # If content is a dict
        text_content = content_data.get("text", "")
        image_url = content_data.get("image", None)
    else:  # If content is a plain string
...
def load_past_messages(user_id):
    """Load past messages for a specific user from DuckDB into cache"""
    
    past_messages[user_id] = {}  # Initialize user dictionary
    conversations[user_id] = {}  # Initialize user dictionary

    results = conns[user_id].execute(
        "SELECT conversation_id, role, content FROM messages ORDER BY timestamp"
    ).fetchall()
    
    for conversation_id, role, content in results:
        # Keep past_messages intact with original content
        message = {"role": role, "content": json.loads(content)}
        
        # Ensure the conversation exists in the user's message cache for past_messages
        if conversation_id not in past_messages[user_id]:
            past_messages[user_id][conversation_id] = []  # Initialize conversation list
        
        past_messages[user_id][conversation_id].append(message)
        
        # Now, parse the content for conversations dictionary
        text, image_url = parse_message_content(content)
        ...
        # Ensure the conversation exists in the user's conversations cache
        if conversation_id not in conversations[user_id]:
            conversations[user_id][conversation_id] = []  # Initialize conversation list
        
        if role != "system":
            conversations[user_id][conversation_id].append(conversation_message)
        #print(conversation_message)
# Example function to validate Google token
def validate_google_token(credential):
    try:
        print("Validating")
        # The audience parameter is the client ID that should match your app's Google API credentials
        audience = '835513582400-psmofpoa40g765kbbc4chm1dvnmefh81.apps.googleusercontent.com'
        
        # Verify and decode the token using google-auth library
        decoded_token = id_token.verify_oauth2_token(credential, Request(), audience)

        print("Validating Successful")
        print(decoded_token)  # Log the decoded user data
        return decoded_token  # This contains user data (sub, email, etc.)

    except GoogleAuthError as e:
        # Catch GoogleAuthError if the token is invalid
        ...
        raise Exception(f"Error while validating Google token: {e}")
...
def login():
    try:
        # Get the credential (Google token) from the request
        credential = request.json['credential']
        
        # Validate the Google token
        user_data = validate_google_token(credential)

        # Create a JWT token for your session
        token = jwt.encode(
            {'user_id': user_data['sub'], 'exp': datetime.utcnow() + timedelta(days=7)},
        ...
        # Handle missing 'credential' key in request body
        return jsonify({'error': 'Missing credential field in request'}), 400
    except Exception as e:
        # Handle general errors like invalid token, JWT issues, etc.
...
def verify_token():
    try:
        ...
        # Get user data based on the decoded token
        user_data = get_user_by_id(decoded_token['user_id'])
        
        return jsonify({'user': user_data, 'userId': decoded_token['user_id']}), 200
    except Exception as e:
...
def handle_connect():
    ...
    db_path = os.path.join(USER_DB_DIR, f"{user_id}.duckdb")

    conn = connect_to_duckdb(db_path)
    global conns
    conns[user_id] = conn
    if conn is None:
        emit('error', {'message': 'Failed to connect to database'})
        return
    
    # Create tables if they do not exist
    conns[user_id].execute("""
...
def upload_file():
    ...
    if file:
        ...
        #Temp link
        ngrok_public_url = "http://localhost:5000"
...
def handle_get_conversation_ids(data):
    ...
    conversation_ids = conns[user_id].execute("SELECT conversation_id FROM conversations").fetchall()
    conversation_ids = [row[0] for row in conversation_ids]  # Convert tuples to list

    print("Convo ids: ", conversation_ids)
    #conns[user_id].close()

    # Send back conversation IDs
...
def handle_get_past_messages(data):
    ...
    # Fetch past messages for the selected conversation (empty list if not found)
    convo = conversations.get(user_id, {}).get(conversation_id, [])
    
    # Debugging: check the conversation data
    print("Past messages: ", user_id, conversation_id, convo)
    
    # Structure the data in the format you want to emit
    response_data = {
        'conversation_id': conversation_id,
        'messages': convo  # Send the messages as part of an object
    }

    # Emit the structured data
    emit('past_messages', response_data)
...
class DraggableLabelApp(MDApp):
    ...
    def add_message(self, isVisionModel, role, user_text=None, image=None, user_id=None, conversation_id=None):
        """Keeps the original logic intact and also stores messages in the database"""
        global past_messages

        if user_id not in past_messages:
            past_messages[user_id] = {}

        if conversation_id not in past_messages[user_id]:
            past_messages[user_id][conversation_id] = []

            # Ensure the conversation ID exists in the conversations table
            conns[user_id].execute(
            ...
            system_message = {
                "role": "system",
                "content": "Your role is to assist users by providing information, answering questions, and engaging in conversations on various topics."
            }
            past_messages[user_id][conversation_id].append(system_message)

            # Insert the system message into the database
            conns[user_id].execute(
                "INSERT INTO messages (conversation_id, role, content) VALUES (?, ?, ?)",
                [conversation_id, system_message["role"], json.dumps(system_message["content"])]
            )

        # Add the assistant/user message based on role and input
        if role == "assistant":
            past_messages[user_id][conversation_id].append({
                "role": role,
                "content": user_text
            })
        ...
        if isVisionModel:
            if image is not None:
                message = {"role": role, "content": [{"type": "text", "text": user_text}, {"type": "image_url", "image_url": {"url": image}}]}
            else:
                print("Image does not exists")
                message = {"role": role, "content": [{"type": "text", "text": user_text}]}
        else:
            print("Not vision model")
            message = {"role": role, "content": user_text}
        ...
        # Store message in DuckDB
        conns[user_id].execute(
            "INSERT INTO messages (conversation_id, role, content) VALUES (?, ?, ?)",
            [conversation_id, message["role"], json.dumps(message["content"])]
        )
        print("Serialized content to insert:", json.dumps(message["content"]))
    ...
    def continue_conversation(self, model=None, user_text=None, context=None, user_id=None, image=None, conversation_id=None):
        ...
        if user_text is not None:
            model = "meta-llama/Llama-3.2-11B-Vision-Instruct-Turbo"  # Use your specific model
            ...
            # Create a copy of the past messages for this conversation
            formatted_messages = copy.deepcopy(past_messages[user_id][conversation_id])

            # Iterate through the formatted messages and check for invalid image URLs
            for msg in formatted_messages:
                if isinstance(msg['content'], list):  # Check if the message contains content with images
                    for content_item in msg['content']:
                        if content_item['type'] == 'image_url':
                            image_url = content_item['image_url']['url']
                            if image_url and not image_url.lower().endswith(self.image_extensions):  # Invalid image URL
                                # Remove the image content if the URL is invalid
                                msg['content'] = [item for item in msg['content'] if item['type'] != 'image_url']
                                print(f"Invalid image URL removed: {image_url}")

            # Now send the formatted messages (with any invalid image URLs removed) to the chat API
            chat_completion = client.chat.completions.create(
                messages=formatted_messages,  # Pass the modified list of messages
                model=model,
                max_tokens=5000,
            )
            ...
            # Insert token usage into token_usage table
            conns[user_id].execute("""
                INSERT INTO token_usage (conversation_id, model_name, timestamp, prompt_tokens, completion_tokens, total_tokens)
                VALUES (?, ?, ?, ?, ?, ?)
            """, (conversation_id, model, timestamp, prompt_tokens, completion_tokens, total_tokens))
            print(usage)
            # Add the assistant's response to the conversation
            self.add_message(isVisionModel, "assistant", user_text=response, image=None, user_id=user_id, conversation_id=conversation_id)
